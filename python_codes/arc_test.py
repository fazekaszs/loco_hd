...
def main():
    ...
    prot1_path = Path("./workdir/arc/AF-Arc1-trunc.pdb")
    prot2_path = Path("./workdir/arc/Arc-6gse-trunc.pdb")
    ...
    fig.savefig("./workdir/arc/arc_locohd.png", dpi=300, bbox_inches="tight")

    # Save b-factor labelled structure
    pdb_io = PDBIO()

    for anchor1, anchor2, score in zip(anchors1, anchors2, lchd_scores):

        resi_id1 = primitive_sequence1[anchor1].id.split("/")
        resi_id2 = primitive_sequence1[anchor2].id.split("/")

        atom: Atom
        for atom in prot1_structure[resi_id1[0]][int(resi_id1[1])].get_atoms():
            atom.bfactor = score

        atom: Atom
        for atom in prot2_structure[resi_id2[0]][int(resi_id2[1])].get_atoms():
            atom.bfactor = score

    pdb_io.set_structure(prot1_structure)
    accepted_atoms = [primitive_sequence1[anchor].id for anchor in anchors1]
    pdb_io.save(f"./workdir/arc/arc_blabelled1.pdb", select=AtomSelector(accepted_atoms))

    pdb_io.set_structure(prot2_structure)
    accepted_atoms = [primitive_sequence2[anchor].id for anchor in anchors2]
    pdb_io.save(f"./workdir/arc/arc_blabelled2.pdb", select=AtomSelector(accepted_atoms))
